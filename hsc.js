    ...
    topics:[
      ...
      {id:'std-stats',name:'Statistical Analysis',code:'MS-S4',sub:'Bivariate Data Analysis',free:false,
        ...
        quizzes:[
          ...
          ,{q:'The scatter plot below shows a negative linear relationship. Which value of $r$ best matches it?',
            imgSvg:'<svg viewBox="0 0 420 260" xmlns="http://www.w3.org/2000/svg"><rect x="0" y="0" width="420" height="260" fill="transparent"/><g stroke="rgba(255,255,255,0.12)" stroke-width="1"><line x1="52" y1="30" x2="52" y2="220"/><line x1="52" y1="220" x2="392" y2="220"/><line x1="52" y1="60" x2="392" y2="60"/><line x1="52" y1="90" x2="392" y2="90"/><line x1="52" y1="120" x2="392" y2="120"/><line x1="52" y1="150" x2="392" y2="150"/><line x1="52" y1="180" x2="392" y2="180"/></g><g fill="rgba(245,166,35,0.85)" stroke="rgba(245,166,35,0.95)" stroke-width="1"><circle cx="70" cy="70" r="5"/><circle cx="105" cy="85" r="5"/><circle cx="130" cy="95" r="5"/><circle cx="160" cy="110" r="5"/><circle cx="190" cy="120" r="5"/><circle cx="220" cy="135" r="5"/><circle cx="250" cy="150" r="5"/><circle cx="280" cy="165" r="5"/><circle cx="310" cy="175" r="5"/><circle cx="345" cy="190" r="5"/></g><g fill="rgba(255,255,255,0.55)" font-family="DM Sans, Arial" font-size="12"><text x="200" y="246">x</text><text x="28" y="36">y</text></g></svg>',
            opts:['$r \\approx -0.85$','$r \\approx -0.20$','$r \\approx 0.70$','$r \\approx 0$'],
            ans:0,
            explain:'The points trend downwards with relatively little scatter, so $r$ is negative and fairly close to $-1$ (around $-0.8$ to $-0.9$).'}
        ]
      },
...
function renderTopicContent() {
  ...
  const locked = !topic.free && !isPro(state.user);
...
function buildQuizImage(q){
  const img = typeof q.img === 'function' ? q.img() : q.img;
  const svg = typeof q.imgSvg === 'function' ? q.imgSvg() : q.imgSvg;
  if (svg) return `<div class="q-img">${svg}</div>`;
  if (img) return `<div class="q-img"><img src="${img}" alt="Question graph"/></div>`;
  return '';
}
...
function buildQuiz(topic, locked) {
  ...
  const pKey = `${state.course}-${topic.id}`;
  const mastered = Math.min(state.progress[pKey] || 0, total);
...
// =============================================
// USERS / AUTH (prototype — local storage)
// =============================================
const LS = {
  users: 'ml-users',
  user: 'ml-user',
...
function planLabel(u) {
  ...
  if (u.plan === 'trial' && u.trialEndsAt && Date.now() < u.trialEndsAt) return 'trial';
  return 'free';
}
function fmtDate(ts) {
  if (!ts) return '—';
...
function bumpStreak() {
  ...
  if (!last) state.streak = 1;
    ...
    state.streak = (last === ymd(y)) ? (state.streak + 1) : 1;
  }
  localStorage.setItem(LS.lastActive, today);
  save();
}

// SHA-256 (so we don't store plaintext passwords in the demo)
...
// Seed a default admin so the staff console works out of the box
function ensureSeedUsers() {
  if (localStorage.getItem(LS.users)) return;
  setUsers([{
    id: 'u_admin',
    name: 'Staff Admin',
    email: 'admin@mathletica.cc',
    role: 'admin',
    plan: 'pro',
    trialEndsAt: null,
    passHash: 'ac9689e2272427085e35b9d3e3e8bed88cb3434828b43b86fc0596cad4c6e270',
    createdAt: Date.now()
  }]);
}

// Keep logged-in user synced to the stored record (plan changes etc.)
function syncUserFromStore() {
  if (!state.user) return;
  const u = getUsers().find(x => x.email.toLowerCase() === state.user.email.toLowerCase());
  if (!u) {
    state.user = null;
...
function refreshNavAuth() {
  ...
  if (!navUser || !btnIn || !btnUp || !btnOut) return;

  if (u) {
    const label = planLabel(u);
    const dotClass = (u.role === 'admin') ? 'admin' : (label === 'pro' || label === 'trial') ? '' : 'free';
    navUser.style.display = '';
    navUser.innerHTML = `<span class="dot ${dotClass}"></span><span class="mono">${u.email}</span><span style="color:var(--text3)">${u.role === 'admin' ? 'staff' : label}</span>`;
...
async function handleAuth(e, type) {
  ...
  if (type === 'login') {
    ...
    if (!u) return toast('No account found for that email.', 'error');
    const h = await sha256(pass);
    if (h !== u.passHash) return toast('Incorrect password.', 'error');
  ...
  if (!name || !email || pass.length < 8) return toast('Please fill all fields (password min 8 chars).', 'error');

  const users = getUsers();
  if (users.some(x => x.email.toLowerCase() === email)) return toast('That email already has an account.', 'error');
  ...
  state.user = publicUser(newUser);
  localStorage.setItem(LS.user, JSON.stringify(state.user));
  closeModal();
  refreshNavAuth();
  renderAdmin();
  renderTopicContent();
  toast('Account created! Enjoy your free trial 🎉', 'success');
}

// =============================================
// ADMIN (prototype — local storage)
// =============================================
function renderAdmin() {
  ...
  if (!state.user) {
    gate.innerHTML = `Sign in as staff to access the admin panel.`;
    panel.style.display = 'none';
    return;
  }
  if (state.user.role !== 'admin') {
    gate.innerHTML = `You are signed in as <span class="mono">${state.user.email}</span>. Admin access is staff-only.`;
    panel.style.display = 'none';
    return;
  }
  gate.innerHTML = `Signed in as staff: <span class="mono">${state.user.email}</span>`;
  panel.style.display = '';
  renderAdminUsers();
}

function chip(label, cls) {
  return `<span class="admin-chip ${cls}">${label}</span>`;
}
...
function renderAdminUsers() {
  ...
  const users = getUsers()
    .filter(u => (u.email + ' ' + (u.name || '')).toLowerCase().includes(q))
    .sort((a, b) => (a.role === 'admin' ? -1 : 1) - (b.role === 'admin' ? -1 : 1));

  body.innerHTML = users.map(u => {
    const pl = planLabel(u);
    const planChip = pl === 'pro' ? chip('Pro', 'pro') : pl === 'trial' ? chip('Trial', 'trial') : chip('Free', 'free');
    const roleChip = u.role === 'admin' ? chip('Admin', 'admin') : chip('Student', 'student');
    const trial = (u.plan === 'trial') ? fmtDate(u.trialEndsAt) : '—';

    const canDelete = u.email.toLowerCase() !== 'admin@mathletica.cc';
    return `
      <tr>
        <td><div style="font-weight:700;color:var(--text)">${u.name || '—'}</div><div class="mono" style="font-size:12px;color:var(--text3)">${u.email}</div></td>
        <td>${roleChip}</td>
        <td>${planChip}</td>
        <td class="mono">${trial}</td>
        <td>
          <div class="admin-row-actions">
            <button class="btn btn-ghost btn-sm" onclick="adminTogglePlan('${u.id}')">${pl === 'pro' ? 'Set Free' : 'Set Pro'}</button>
            <button class="btn btn-ghost btn-sm" onclick="adminStartTrial('${u.id}')">Start/Extend Trial</button>
            <button class="btn btn-ghost btn-sm" onclick="adminResetPassword('${u.id}')">Reset Password</button>
            ${canDelete ? `<button class="btn btn-outline btn-sm" onclick="adminDeleteUser('${u.id}')">Delete</button>` : ``}
          </div>
        </td>
      </tr>
    `;
...
function adminTogglePlan(id) {
  updateUserById(id, (u) => {
    const pl = planLabel(u);
    if (pl === 'pro') { u.plan = 'free'; u.trialEndsAt = null; }
    else { u.plan = 'pro'; u.trialEndsAt = null; }
    return u;
  });
  toast('Plan updated.', 'success');
}

function adminStartTrial(id) {
  updateUserById(id, (u) => {
    u.plan = 'trial';
    u.trialEndsAt = Date.now() + 7 * 24 * 60 * 60 * 1000;
    return u;
  });
  toast('Trial started/extended.', 'success');
}
...
async function adminResetPassword(id) {
  if (!state.user || state.user.role !== 'admin') return toast('Admin only.', 'error');
  const temp = randPass();
  const users = getUsers();
  const idx = users.findIndex(u => u.id === id);
  if (idx === -1) return;
  users[idx].passHash = await sha256(temp);
  setUsers(users);
  syncUserFromStore();
  refreshNavAuth();
  renderAdmin();
  toast(`New password: ${temp}`, 'info');
}
...
function adminDeleteUser(id) {
  ...
  if (state.user && state.user.email.toLowerCase() === u.email.toLowerCase()) signOut();
  renderAdmin();
...
async function adminCreateUser(e) {
  ...
  const plan = document.getElementById('aPlan')?.value || 'free';

  if (!name || !email || pass.length < 6) return toast('Fill all fields (password min 6 chars).', 'error');
  const users = getUsers();
  if (users.some(x => x.email.toLowerCase() === email)) return toast('Email already exists.', 'error');
  ...
  const u = {
    ...
    trialEndsAt: plan === 'trial' ? (now + 7 * 24 * 60 * 60 * 1000) : null,
    passHash: await sha256(pass),
    createdAt: now
  };
  users.push(u);
...
function exportUsers() {
  if (!state.user || state.user.role !== 'admin') return toast('Admin only.', 'error');
  const data = JSON.stringify(getUsers().map(publicUser), null, 2);
  ...
  toast('Exported users JSON.', 'success');
}
